#!/usr/bin/env python3
"""
RAG Evaluation - CPU Flat Index (Uncompressed, Perfect Recall)

Evaluates retrieval performance using the uncompressed Flat FAISS index on CPU.
- Uses vLLM + Qwen embeddings (same as embed_texts.py)
- Loads Flat index on CPU for exact nearest neighbor search
- Evaluates Recall@K and MRR@K metrics
- No reranker (pure vector similarity retrieval)
- Perfect recall but slower than IVF-PQ
...
FAISS_INDEX = Path("indexes/faiss_qwen06_flat_cpu.index")
ID_MAPPING = Path("indexes/patent_id_mapping_cpu.npy")
MODEL_NAME = "Qwen/Qwen3-Embedding-0.6B"
OUTPUT_FILE = Path("data/question_generation/rag_eval_cpu_flat.csv")
TOP_K = [1, 5, 10, 20, 50, 100]
NUM_THREADS = 32  # CPU threads for FAISS search
...
def main():
    print("="*80)
    print("RAG Evaluation - CPU Flat Index (Uncompressed, Perfect Recall)")
    print("="*80)
    
    # Check GPU for embedding model
    if not torch.cuda.is_available():
        print("WARNING: No GPU available for embedding model, this will be very slow")
    else:
        print(f"GPU available for embeddings: {torch.cuda.get_device_name(0)}")
    ...
    print(f"\nLoading FAISS Flat index: {FAISS_INDEX}")
    print("(This may take several minutes for large indexes)")
    index = faiss.read_index(str(FAISS_INDEX))
    print(f"✓ Index loaded on CPU ({index.ntotal:,} vectors)")
    
    # Set CPU threads
    faiss.omp_set_num_threads(NUM_THREADS)
    print(f"✓ Using {NUM_THREADS} CPU threads for search")
    
    # Load ID mapping
    print(f"\nLoading ID mapping: {ID_MAPPING}")
    ...
    print("(Flat index provides exact nearest neighbors - perfect recall)")
    max_k = max(TOP_K)
    ...
    print("\n" + "="*80)
    print("RESULTS (Perfect Recall Baseline)")
    print("="*80)
    for k in TOP_K:
    ...
    print("\nEvaluation complete!")
    print("\nNote: This Flat index provides exact nearest neighbors (100% recall)")
    print("      Compare with IVF-PQ results to see compression trade-offs")
